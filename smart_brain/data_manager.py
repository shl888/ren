...
class DataManager:
    ...
    async def save_listen_key(self, exchange: str, listen_key: str):
        """保存listenKey（供http模块调用）"""
    ...
    async def clear_stored_data(self, data_type: str = None):
        ...
        try:
            ...
            if data_type == 'market':
                ...
                self.last_account_time = None
                self.last_trade_time = None
                message = f"清空所有数据，市场数据{before_stats['market_data_count']}条，私人数据{before_stats['private_data_count']}条"
