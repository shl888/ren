...
- 2024-03-15：修复初始化日志问题，检测与日志逻辑统一
==================================================
"""
...
class Database:
   ...
   def _init_database(self):
       ...
       初始化数据库 - 修复版
       ==================================================
       【修复】2024-03-15：统一检测与日志逻辑
       先检测表是否存在，根据检测结果决定创建或跳过，并打印对应日志
       ==================================================
       """
       try:
           # 持仓表
           if self._table_exists('active_positions'):
           ...
           # 历史表
           if self._table_exists('closed_positions'):
   ...
   def _table_exists(self, table_name: str) -> bool:
       ...
       检测表是否存在
       ==================================================
       用 SELECT 1 检测，比查 sqlite_master 更靠谱
       检测逻辑与 SQL 的 IF NOT EXISTS 底层一致
       
       :param table_name: 表名
       :return: True=存在, False=不存在
       """
       try:
           self._run_sql(f"SELECT 1 FROM {table_name} LIMIT 1")
           return True
       except Exception:
           return False
