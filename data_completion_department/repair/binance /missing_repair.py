...
class BinanceMissingRepair:
    ...
    async def _step5_calc_fields(self):
        ...
        direction = cache.get(FIELD_OPEN_DIRECTION)  # "LONG" 或 "SHORT"
        ...
        # 2. 最新价保证金
        if leverage > 0:
            latest_margin = latest_price * position_size / leverage
        else:
            latest_margin = 0
        ...
        # 3. 标记价浮盈百分比
        if open_margin > 0:
            mark_pnl_percent_value = mark_pnl * 100 / open_margin
        else:
            mark_pnl_percent_value = 0
        cache[FIELD_MARK_PNL_PERCENT] = mark_pnl_percent_value
        ...
        if direction == "LONG":
            ...
            latest_pnl = latest_position_value - open_position_value
            latest_pnl_percent_value = latest_pnl * 100 / open_margin
            
        else:  # direction == "SHORT"
            # 空头
            mark_pnl_percent = (open_price - mark_price) * 100 / open_price
            latest_pnl_percent = (open_price - latest_price) * 100 / open_price
            latest_pnl = open_position_value - latest_position_value
            latest_pnl_percent_value = latest_pnl * 100 / open_margin
        ...
        cache[FIELD_LATEST_PNL_PERCENT] = latest_pnl_percent_value
        
        logger.info(f"   计算结果 - 最新价仓位价值: {latest_position_value:.2f}, "
