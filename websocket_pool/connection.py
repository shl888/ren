"""
单个WebSocket连接实现 - 支持角色互换 + 自动重连 + 资源清理
"""
import asyncio
import json
import logging
...
class WebSocketConnection:
    ...
    def __init__(
        ...
        self._reconnect_task = None  # 🚨 新增：重连任务
    ...
    async def connect(self):
        ...
        try:
            ...
            self.reconnect_count = 0
            self._shutting_down = False  # 🚨 重置关闭标志
    ...
    def _get_delay_for_warm_standby(self):
        ...
        try:
            ...
            if len(parts) >= 3:
                index = int(parts[-1])
                return 15 + (index * 5)  # 🚨 调整：从10秒改为15秒
        except:
            pass
        return 15
    ...
    async def switch_role(self, new_role: str, new_symbols: list = None):
        ...
        try:
            ...
            # 🚨 修复：立即更新 is_active 标志
            self.is_active = (new_role == ConnectionType.MASTER)
    ...
    async def _subscribe_binance(self):
        ...
        try:
            ...
            self.subscribed = True
            logger.info(f"[{self.connection_id}] 订阅完成")
    ...
    async def _handle_disconnection(self):
        ...
        if not self._shutting_down:
            logger.warning(f"[{self.connection_id}] 启动自动重连任务")
    ...
    async def _auto_reconnect(self):
        ...
        for attempt in range(3):
            if self._shutting_down:
                logger.info(f"[{self.connection_id}] 正在关闭，取消自动重连")
                return
            
            logger.warning(f"[{self.connection_id}] 尝试自动重连 ({attempt+1}/3)")
            await asyncio.sleep(5 * (attempt + 1))  # 递增延迟
            
            if await self.connect():
                logger.info(f"[{self.connection_id}] 自动重连成功")
                return
        
        logger.error(f"[{self.connection_id}] 自动重连失败，放弃")
    ...
    async def disconnect(self):
        """断开连接 - 修复任务泄露"""
        logger.info(f"[{self.connection_id}] 正在断开连接...")
        ...
        if self.delayed_subscribe_task:
            self.delayed_subscribe_task.cancel()
            try:
                await self.delayed_subscribe_task
